...
Metrics_testdat <- function(testdat , baseline = "False"){
  # Cal the 6 matrix to evalute the performance of our model based on a testdat contains y and y hat
  # Arg:
  # testdat should be a data.frame contain at least the following cols:
  # y: response variable (eg: FUT_12W_RLT_RET)
  # y_hat: the estimated response variable from our model
  ...
  # NOTE: if it's only for baseline model, you input dataframe is allowed to not contain 'y_hat' column
  # baseline: default is "False". Assign it to "True" when you want to see the performance of baseline model
  # Return:
  # a 6*2 matrix, each row represents for each metric in the order of spearman corrlation avg, 
  ...
  if(baseline){ # if it's only for baseline model, we need to assign a col y_hat
  ...
  # 2.1 Spearman correlation average
  corr_date <- testdat %>% 
    group_by(DATE) %>% 
  ...
  # 2.2 Spearman correlation consistency - across groups
  corr_group <- testdat %>% 
  ...
  # 2.3. Spearman correlation consistency - across time
  ## get the % of postive value in each col (cor, cor_bsl) in corr_group
  corr_date <- corr_date %>%
    mutate(cor = ifelse(cor > 0, 1, 0),
           cor_diff = ifelse(cor_diff > 0, 1, 0))
